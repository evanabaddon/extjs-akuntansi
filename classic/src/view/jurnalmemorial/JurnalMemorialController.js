Ext.define('Admin.view.jurnalmemorial.JurnalMemorialController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.jurnalmemorial',

    init: function() {
        var me = this;
        var viewType = me.getView().viewType;
        if(viewType && viewType!='webdesktop') {
            console.log('jurnalmemorial-init');
    ...
    onAddButtonClick: function(btn) {
        ...
        if(viewType=='webdesktop') {   
            ...
            this.setCurrentView('jurnalmemorial-edit', {
    ...
    onLoadData: function(form) {
        var win = this.getView();
        var viewType = win.viewType;
        var form = win.down('jurnalmemorial-edit');
        ...
        if(idEdit) {
            form.load({
                //waitMsg: 'Loading...',
                method: 'GET',
                url: './server/public/jurnalmemorial/'+idEdit+'/load',
                success: function (frm, action) {
                    var json = Ext.JSON.decode(action.response.responseText);
                    var grid = form.down('jurnalmemorial-edit-detail');
    ...
    onDeleteButtonClick: function(btn) {
        ...
        Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk hapus data yang dipilih?', function(btn,text) {
            if(btn=='yes') {
                ...
                Ext.Ajax.request({
                    method:'GET',
                    url: './server/public/jurnalmemorial/'+gridpanel.idSelect+'/delete',
    ...
    onSaveButtonClick: function(btn) {
        ...
        var editDetail = form.down('jurnalmemorial-edit-detail');
        var data_detail = editDetail.getDetail();

        if(editDetail.total_debet!=editDetail.total_kredit) {
            Ext.MessageBox.show({
                title: 'Kesalahan',
                msg: 'Total debet dan total kredit harus sama.',
                buttons: Ext.MessageBox.OK,
                icon: Ext.MessageBox.ERROR
            });
        ...
        if(form.getForm().isValid()) {
            Ext.MessageBox.confirm('Konfirmasi', 'Yakin untuk proses simpan data?', function(btn, text) {
                if(btn=='yes') {
                    ...
                    form.getForm().submit({
                        method:'POST',
                        url: './server/public/jurnalmemorial/'+saveUrl,
                        params: {
                            detail: data_detail,
                            total: editDetail.total_kredit
                        },
                        waitMsg: 'Simpan...',
                        success:function(frm, action) {
                            var json = Ext.JSON.decode(action.response.responseText);
